...
// given IDBDatabase.
function lookupInDatabase(db, storeName, url) {
    return new Promise((resolve, reject) => {
...
function storeInDatabase(db, module, storeName, url) {
...
(function (root, factory) {
    ...
    function __initialize(__wasm_module, __load_asynchronously) {
        ...
        const __imports = {
            env: {
                ...
                "__extjs_c324d9292a5a2f33cd26652722c3cc361e335e97": function ($0, $1) {
                    $1 = Module.STDWEB_PRIVATE.to_js($1);
                    Module.STDWEB_PRIVATE.from_js($0, (function () {
                        return ($1).createDocumentFragment();
                    })());
                },
        ...
        if (__load_asynchronously) {
            return WebAssembly.instantiate(__wasm_module, __imports)
                .then(instance => {
                    ...
                    console.log("Finished loading Rust wasm module 'rust'");
                    return Module.exports;
                })
                .catch(error => {
                    console.log("Error loading Rust wasm module 'rust':", error);
                    throw error;
                });
        } else {
    ...
    if (typeof window === "undefined" && typeof process === "object") {
        ...
        const wasm_path = path.join(__dirname, url);
        const buffer = fs.readFileSync(wasm_path);
        const mod = new WebAssembly.Module(buffer);

        return __initialize(mod, false);
    } else {
        instantiateCachedURL(wasmCacheVersion, url).then(mod => __initialize(mod, true));
    }
}));